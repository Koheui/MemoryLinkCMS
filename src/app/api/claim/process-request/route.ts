...
export async function POST(request: NextRequest) {
  try {
    ...
    const { requestId, tenant, lpId, idToken } = body;

    // 入力検証
    if (!requestId || !tenant || !lpId || !idToken) {
    ...
    // テナントとLP IDの一致確認
    if (claimRequestData.tenant !== tenant || claimRequestData.lpId !== lpId) {
      return NextResponse.json(
        { error: 'クレーム要求のパラメータが一致しません' },
        { status: 400 }
      );
    }
    ...
    console.error('クレーム要求処理エラー:', error);
